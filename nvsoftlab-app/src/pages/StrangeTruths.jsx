  ...
  const features = [
    {
      ...
      description: "Hundreds of facts so wild you won't believe they're real — until you see the proof.",
    },
    {
      icon: <Gamepad2 className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Fun Game Modes",
      description: "Multiple categories and custom decks so every session feels different.",
    },
    {
      icon: <Heart className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Debate & Laugh",
      description: "The reveal is where it gets loud. Defend your guess or admit you had no idea.",
    },
    {
    ...
    },
    {
      ...
      description: "New facts added regularly — you'll never run out of surprises.",
    },
    {
      icon: <Lock className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Lifetime Access",
      description: "One purchase, everything unlocked. No subscriptions, no limits.",
    },
  ];
  ...
  const howItWorks = [
    {
      ...
      description: "Browse themed decks or build your own — from science to history to the downright bizarre.",
    },
    {
      step: "2",
      title: "Make Your Guess",
      description: "Swipe right for true, left for false. Sounds easy — until the facts get weird.",
    },
    {
      step: "3",
      title: "Face the Truth",
      description: "See the real answer and watch the room erupt. Wrong guesses lead to the best arguments.",
    },
  ];

  const perfectFor = [
    { icon: <Tv className="w-6 h-6 text-white" />, title: "Watch Parties", desc: "Perfect way to kill time while waiting for the new season" },
    { icon: <Bed className="w-6 h-6 text-white" />, title: "Sleepovers", desc: "Turn off the lights and make the night a little spookier" },
    { icon: <Car className="w-6 h-6 text-white" />, title: "Road Trips", desc: "Play offline anywhere — even in the middle of nowhere" },
    { icon: <Users className="w-6 h-6 text-white" />, title: "Ice Breakers", desc: "Find out who in your group knows the strangest truths" },
  ];
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div className="text-center mb-16 sm:mb-20">
        ...
        <p className="text-base sm:text-lg md:text-xl text-amber-100/80 mb-8 max-w-3xl mx-auto leading-relaxed px-2">
          Swipe through jaw-dropping facts and decide — is it real or completely made up? The kind of trivia that sparks heated debates, wild theories, and makes everyone question everything.
        </p>
          ...
          {howItWorks.map((item, index) => (
            <Card key={index} className={`${CARD_BG} backdrop-blur-sm border ${BORDER} shadow-lg ${GLOW} hover:shadow-xl transition-all duration-300 group`}>
              <CardContent className="p-6 sm:p-8">
                ...
                <h3 className="text-lg sm:text-xl font-black text-white mb-2 group-hover:text-amber-400 transition-colors duration-300">{item.title}</h3>
                <p className="text-sm sm:text-base text-amber-100/70 leading-relaxed">{item.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <div className="mb-16 sm:mb-20">
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">What Makes It Addictive</h2>
          <p className="text-amber-200/70">The kind of details that keep you coming back</p>
        </div>
          ...
          {features.map((feature, index) => (
            <Card key={index} className={`${CARD_BG} backdrop-blur-sm border ${BORDER} shadow-lg ${GLOW} hover:shadow-xl transition-all duration-300 group`}>
              <CardContent className="p-4 sm:p-6">
                ...
                <h3 className="text-lg sm:text-xl font-black text-white mb-2 group-hover:text-amber-400 transition-colors duration-300">{feature.title}</h3>
                <p className="text-sm sm:text-base text-amber-100/70 leading-relaxed">{feature.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      {/* Download CTA */}
      <div className={`text-center bg-gradient-to-br from-amber-950/60 to-orange-950/40 border ${BORDER} rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12`}>
        <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">Think You Know Everything?</h2>
        <p className="text-amber-100/70 mb-8 max-w-2xl mx-auto">Download Strange Truths and prove it. Fair warning — you'll be wrong more than you think.</p>
