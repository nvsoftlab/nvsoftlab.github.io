  ...
  const features = [
    {
      ...
      title: "700+ Questions",
      description: "From sweet to spicy — every question designed to spark something real.",
    },
    {
      icon: <Users className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Mood-Based Categories",
      description: "Romantic, playful, or daring. Pick the vibe that fits your night.",
    },
    {
      icon: <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Share Your Stories",
      description: "The best moments happen when someone says 'okay, let me explain\u2026'",
    },
    {
    ...
    },
    {
      ...
      description: "Fresh questions every time — the AI keeps it unpredictable.",
    },
    {
      icon: <Lock className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      title: "Access All Categories",
      description: "Unlock everything. Romantic, spicy, exclusive — it's all yours.",
    },
  ];
  ...
  const howItWorks = [
    {
      ...
      description: "Romantic, playful, or spicy — set the mood for your evening.",
    },
    {
      step: "2",
      title: "Answer Honestly",
      description: "Both swipe honestly. Green if you've done it, red if not — no hiding allowed.",
    },
    {
      step: "3",
      title: "Share Your Stories",
      description: "The stories that come out are the best part. Laugh, blush, and learn something new.",
    },
  ];

  const perfectFor = [
    { icon: <Heart className="w-6 h-6 text-white" />, title: "Date Nights", desc: "Transform any evening into an adventure" },
    { icon: <Calendar className="w-6 h-6 text-white" />, title: "Anniversaries", desc: "Celebrate with conversations that actually matter" },
    { icon: <Moon className="w-6 h-6 text-white" />, title: "Long Distance", desc: "Stay connected from anywhere in the world" },
    { icon: <Sparkles className="w-6 h-6 text-white" />, title: "New Couples", desc: "Skip the small talk, get to the real stuff" },
  ];
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div className="text-center mb-16 sm:mb-20">
        ...
        <p className="text-base sm:text-lg md:text-xl text-rose-100/80 mb-8 max-w-3xl mx-auto leading-relaxed px-2">
          700+ questions that go way beyond 'how was your day.' Designed for couples who want deeper conversations, unexpected confessions, and nights they'll actually remember.
        </p>
        ...
        <div className="flex flex-row items-center justify-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <Button asChild size="lg"
            ...
            <a href="https://apps.apple.com/us/app/exposed-game-have-you-ever/id6751416602" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 justify-center">
          ...
          {howItWorks.map((item, index) => (
            <Card key={index} className={`${CARD_BG} backdrop-blur-sm border ${BORDER} shadow-lg ${GLOW} hover:shadow-xl transition-all duration-300 group`}>
              <CardContent className="p-6 sm:p-8">
                ...
                <h3 className="text-lg sm:text-xl font-black text-white mb-2 group-hover:text-rose-400 transition-colors duration-300">{item.title}</h3>
                <p className="text-sm sm:text-base text-rose-100/70 leading-relaxed">{item.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <div className="mb-16 sm:mb-20">
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">Perfect For</h2>
          <p className="text-rose-200/70">Every stage of your relationship</p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {perfectFor.map((item, index) => (
            <Card key={index} className={`${CARD_BG} border ${BORDER} shadow-lg hover:shadow-xl transition-all duration-300`}>
              <CardContent className="p-6 text-center">
                <div className={`w-12 h-12 bg-gradient-to-r ${ACCENT} rounded-full flex items-center justify-center mx-auto mb-4`}>
                  {item.icon}
                </div>
      ...
      <div className="mb-16 sm:mb-20">
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">Why Couples Love It</h2>
          <p className="text-rose-200/70">Everything you need for unforgettable evenings</p>
        </div>
          ...
          {features.map((feature, index) => (
            <Card key={index} className={`${CARD_BG} backdrop-blur-sm border ${BORDER} shadow-lg ${GLOW} hover:shadow-xl transition-all duration-300 group`}>
              <CardContent className="p-4 sm:p-6">
                ...
                <h3 className="text-lg sm:text-xl font-black text-white mb-2 group-hover:text-rose-400 transition-colors duration-300">{feature.title}</h3>
                <p className="text-sm sm:text-base text-rose-100/70 leading-relaxed">{feature.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      {/* Download CTA */}
      <div className={`text-center bg-gradient-to-br from-rose-950/60 to-fuchsia-950/40 border ${BORDER} rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12`}>
        <h2 className="text-2xl sm:text-3xl font-black text-white mb-3">Your Best Date Night Starts Here</h2>
        <p className="text-rose-100/70 mb-8 max-w-2xl mx-auto">Download Exposed AI and turn any evening into something you'll both remember.</p>
